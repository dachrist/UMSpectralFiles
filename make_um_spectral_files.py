...
def make_um_spectral_files(
    ...
    stellar_spectrum_format=None,
    bands=32,
    estf_dir="/data/jgoyal/kabs/",
    grav=None,
    file_prefix=None,
    file_suffix="",
    atmo_profile=None,
    ...
    if grav > 50.0:
        raise Warning(
            "The specified gravity is rather high.  Are you sure it is in m/s^2"
        )
    ...
    if stellar_spectrum_format == "UM":
        ...
        convert.write_um_spec(
            fname=stellar_spectrum,
            fname_out="{}/stellar_spectrum.txt".format(workdir),
            star_name="Star",
            source="Some source",
        )
    else:
        raise Exception(
            "Unsupported stellar spectrum format. UM and ATMO are the only options."
        )
    ...
    for x in specs:
        if x.upper() not in cols:
            raise Exception(
                "Error: can't find key for {} in column density dictionary.".format(
                    x.upper()
                )
            )
    ...
    def generate_scripts(bandtype):
        ...
        f.write(
            "# 2: Add thermal source function: tabulated between 70 and 4000 K with 1000 points\n"
        )
